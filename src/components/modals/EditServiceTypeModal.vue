...
watch(
    ...
    (newValue) => {
        ...
        if (newValue && props.serviceType) {
            // Предзаполнить форму при открытии
            typeName.value = props.serviceType.name || '';
            error.value = '';
        }
    }
);
...
async function handleUpdate() {
    ...
    try {
        await emit('update', { name: typeName.value.trim() });
        closeModal();
    } catch (err) {
    ...
    <Transition name="modal">
        <div v-if="isVisible" class="modal-overlay" @click="handleBackdropClick">
            <div class="modal-content" @click.stop>
                <h2 class="modal-title">Редактирование вида заявки</h2>
